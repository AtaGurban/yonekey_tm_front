...
const Main = observer(() => {
  ...
  const firstTitleCategory = category.titleCategory.filter(
    (i) => i.number === 1
  );
  const secondTitleCategory = category.titleCategory.filter(
    (i) => i.number === 2
  );
  const thirdTitleCategory = category.titleCategory.filter(
    (i) => i.number === 3
  );
  const fourTitleCategory = category.titleCategory.filter(
    (i) => i.number === 4
  );
  const fiveTitleCategory = category.titleCategory.filter(
    (i) => i.number === 5
  );
  const sixTitleCategory = category.titleCategory.filter((i) => i.number === 6);
  useEffect(() => {
    (async function () {
      await getBusiness().then(async (data) => {
        setBusiness(data);
      });
